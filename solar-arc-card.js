...
<svg viewBox="${viewBox}" xmlns="http://www.w3.org/2000/svg" style="display:${this._config.arc_show ? 'block' : 'none'}">
  ...
  _updateDOM() {
    ...
    if (isDay) {
      ...
      if (arcRem && this._config.arc_path_color_remaining) {
    ...
    if (cfg.arc_text_color) {
      ['#grd-val','#hse-val','#bat-val','#rise-lbl','#set-lbl'].forEach(sel => {
...
class SolarArcCardEditor extends HTMLElement {
  ...
  _render() {
    ...
    <style>
      ...
      .sac-ed .sac-cp   { width: 44px; height: 34px; padding: 2px; box-sizing: border-box;
